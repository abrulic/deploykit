...
- **`generate`** — reads [`deploykit.config.ts`](#the-config-file) and re-emits every Dockerfile, `fly.toml`, and the workflow from it. Use it after you edit the config by hand. Regenerating is the point, so this **overwrites those files even if you've customized them** — it lists what it will overwrite and asks first (pass `--yes` to skip the prompt, or `--dry-run` to just see the list). The config file itself is never touched; it's the input.
...
| `--dry-run` | init, generate | Detect and print the plan, but write nothing. |
...
| `--force` | init | Overwrite existing generated files instead of skipping them. (`generate` always overwrites.) |
